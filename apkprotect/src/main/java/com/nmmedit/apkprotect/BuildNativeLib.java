...
public class BuildNativeLib {
    //库名称，可不可以直接使用随机名字来生成？在最开始执行的方法中，判断是否为空，如果是空则随机生成一个名字。
    public static String NMMP_NAME = null;

    //
    //虚拟机库名称,如果cmake里配置为静态库,这个可以忽略
//    public static final String VM_NAME = "wxjnmmvm";

    public static void init_nmmp_name() {
        if (NMMP_NAME == null) {
            NMMP_NAME = RandomUtils.encrypt("nmmap");
        }
        System.out.println("so的名字是"+NMMP_NAME);
    }
    ...
    public static Map<String, Map<File, File>> generateNativeLibs(@Nonnull File outDir,
        ...
        if (isEmpty(cmakePath)) {
            ...
            System.err.println("No CMAKE_PATH，Default is " + cmakePath);
        }
