...
//    private Opcodes opcodes;
    ...
    private static String nativeClassName = null;

    //每个处理过的class,需要调用这个类里的注册函数,注册函数名和classes.dex相关
    public String getRegisterNativesClassName() {
        if (nativeClassName == null) {
            nativeClassName = "com/" + RandomUtils.generateRandomClaName() + "/"+RandomUtils.encrypt("NativeUtil");
        }
        //System.out.println("类名是"+nativeClassName);
        return nativeClassName;
//        return "com/wxj/NativeUtil";
    }

    private static String obsMethodName = null;

    public String getRegisterNativesMethodName() {
        // 试试每次打包都随机一个名字
        if (obsMethodName == null) {
            obsMethodName = RandomUtils.generateRandomMethodName();
        }
        //System.out.println("方法后缀是"+obsMethodName); getDexName() +
        return getDexName() + obsMethodName;
    }
