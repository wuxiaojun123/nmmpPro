            ...
            if(rule.className.contains(",")) {
                ...
                for (int i = 0;i<split.length;i++) {
                    ...
                    if (str.startsWith("!")) {
                        ...
                        // System.out.println("newStr="+newStr+"---typeRegex="+typeRegex + "-----" + classType);
                ...
                // 替换,之前的字符串
                String newRuleClassName = rule.className.substring(rule.className.lastIndexOf(",")+1);
                typeRegex = toRegex(classNameToType(newRuleClassName));
            } else {
                typeRegex = toRegex(classNameToType(rule.className));
            }
            // typeRegex=Lcom/.*;classType=Lkotlinx/coroutines/scheduling/SchedulerCoroutineDispatcher;
            // System.out.println("typeRegex="+typeRegex + "classType=" + classType);
